...
static void fill_props(GVariantBuilder *b, int id) {
    ...
    const char *label = (id == ID_SHOW) ? "Show Pulse"
                      : (id == ID_QUIT) ? "Quit Pulse" : "";
...
static void menu_method(GDBusConnection *, const char *, const char *,
    ...
    if (g_strcmp0(method, "GetLayout") == 0) {
        ...
        if (g_strcmp0(name, "label") == 0)
            val = g_variant_new_string(id == ID_SHOW ? "Show Pulse"
                                     : id == ID_QUIT ? "Quit Pulse" : "");
...
static GVariant *sni_get_prop(GDBusConnection *, const char *, const char *,
    ...
    if (g_strcmp0(prop, "Id") == 0)            return g_variant_new_string("pulse");
    if (g_strcmp0(prop, "Title") == 0)         return g_variant_new_string("Pulse");
    ...
    if (g_strcmp0(prop, "ToolTip") == 0) {
        ...
        return g_variant_new("(sa(iiay)ss)", g_icon, &icon, "Pulse",
