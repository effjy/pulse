...
// ---------------------------------------------------------------------------
// About dialog (with diskmon's stray-selection workaround)
// ---------------------------------------------------------------------------
static void clear_label_selection(GtkWidget *w) {
    if (GTK_IS_LABEL(w)) gtk_label_select_region(GTK_LABEL(w), 0, 0);
...
static gboolean about_unselect_idle(gpointer data) {
    GtkWidget *d = static_cast<GtkWidget *>(data);
    if (GTK_IS_WIDGET(d)) { clear_label_selection(d); gtk_window_set_focus(GTK_WINDOW(d), nullptr); }
...
static void show_about(GtkWidget *, gpointer data) {
    ...
    gtk_about_dialog_set_program_name(ad, "Pulse");
    gtk_about_dialog_set_version(ad, PULSE_VERSION);
    gtk_about_dialog_set_comments(ad,
        "A unified, real-time monitor for the whole machine.\n\n"
        "• CPU — total & per-core usage, frequency, load averages\n"
        "• Temperature — live CPU package temperature in °C or °F\n"
        "• Memory — RAM & swap usage with breakdown\n"
        "• Disk — per-filesystem usage and live read/write throughput\n"
        "• Network — live up/down speed and active connections\n\n"
        "Tokyo Night theme · Cairo graphs · no dependencies beyond GTK4.");
    ...
    gtk_about_dialog_set_logo_icon_name(ad, "pulse");
    ...
    g_signal_connect(dialog, "map", G_CALLBACK(+[](GtkWidget *w, gpointer){
        g_idle_add(about_unselect_idle, w); }), nullptr);
...
static void activate(GtkApplication *app, gpointer data) {
    ...
    gtk_window_set_title(GTK_WINDOW(s->window), "Pulse");
    gtk_window_set_default_size(GTK_WINDOW(s->window), 1180, 660);
    gtk_window_set_icon_name(GTK_WINDOW(s->window), "pulse");
    ...
    // Header bar with stack switcher (horizontal tabs) + controls
    ...
    gtk_widget_set_tooltip_text(about_btn, "About Pulse");
    ...
    s->tray_active = tray_init(G_APPLICATION(app), "pulse",
...
int main(int argc, char **argv) {
    AppState state;
    GtkApplication *app = gtk_application_new("org.effjy.pulse", G_APPLICATION_DEFAULT_FLAGS);
